...
public class GamepadJoinBehavior : MonoBehaviour
{
    ...
    void Awake()
    {
        SceneManager.activeSceneChanged += InitializeCanvas;
    }
    ...
    void InitializeCanvas(Scene arg0, Scene arg1)
    {
    ...
    void AddPlayer(InputDevice device)
    {
        ...
        GameObject playerCard = playerCards[playerNumberToAdd - 1];


        if (!playerCursor.activeInHierarchy && SceneManager.GetActiveScene().name != "GameEndScreen")
        {
            ...
            playerCard.GetComponent<PlayerCard>().cursor = theCursor.gameObject;
